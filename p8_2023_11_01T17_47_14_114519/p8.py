...
nm1289434_value = small_mapping.get('nm1289434')
nm1289434_value
...
for key in small_mapping.keys():
    if key.startswith("nm"):
        nm_values.append(small_mapping.get(key))
nm_values
...
for key in small_mapping:
    name = small_mapping[key]
    if key.startswith("nm"):
        if name.endswith(" Murphy"):
...
first_actor_id_first_movie = raw_small_movies[0].get('cast')[0]
...
# films = copy.deepcopy(find_specific_movies)
# hitchcock_films = films(movies,"Alfred Hitchcock")
# hitchcock_films
...
# once_upon_a_time_in_movies = find_specific_movies(copy.copy(movies),"Once")
# once_upon_a_time_in_movies

once_upon_a_time_in_movies = find_specific_movies(copy.copy(movies), "Once Upon a Time in")
once_upon_a_time_in_movies
...
romance_mov = genre_dict['Romance']
for movie in romance_mov:
    year = movie['year']
    if year > 2019:
...
doc_movies = genre_dict['Documentary']
for movie in doc_movies:
    movie_time = movie['duration']
    title = movie['title']
    if movie_time > 240:
        docus_longer_than_240_mins.append(title)
docus_longer_than_240_mins
...
blake_lively_movies = find_specific_movies(copy.copy(movies),"Blake Lively")
blake_lively_genre_movies = bucketize_by_genre(blake_lively_movies)
blake_lively_genre = None
blake_lively_count_dict = {}
max_genre = None
for key in blake_lively_genre_movies:
    blake_lively_count_dict[key] = len(blake_lively_genre_movies.get(key))
    if max_genre == None or blake_lively_count_dict[key]>max_genre:
        max_genre = blake_lively_count_dict[key]
        blake_lively_genre = key
blake_lively_genre

# try switching keys and values then use as an idx
...
horror_rating = None 
horror_genre = genre_dict['Horror']
for movie in horror_genre:
     if horror_rating == None or movie['rating'] > horror_rating:
        horror_rating = movie['rating']
for movie in horror_genre:
    if movie['rating'] == horror_rating:
        horror_dir = movie['directors']
        for direct in horror_dir:   
            max_horror_rating_directors.append(direct)
