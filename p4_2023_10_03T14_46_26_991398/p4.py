...
def type_bonus(attack_type, defender):
    ...
    if defender_type2 == "DNE":
        return project.get_type_effectiveness(attack_type, defender_type1)
    else:
        return project.get_type_effectiveness(attack_type, defender_type1) * project.get_type_effectiveness(attack_type, defender_type2)
...
def effective_damage(attacker, defender):
    ...
    attacker_types = get_num_types(attacker)
    if attacker_types == 2:
        attack_damage = damage(attacker, defender)
        attacker_bonus_type1 = type_bonus(project.get_type1(attacker), defender)
        attacker_bonus_type2 = type_bonus(project.get_type2(attacker), defender)
        max_bonus = max(attacker_bonus_type1, attacker_bonus_type2)
        return attack_damage * max_bonus
    
    elif attacker_types == 1:
        attack_damage = damage(attacker, defender)
        attacker_bonus_type1 = type_bonus(project.get_type1(attacker), defender)
        return attack_damage * attacker_bonus_type1
...
def num_hits(attacker, defender):
    #pass # TODO: replace with your code
    #attacker_hp = project.get_hp(attacker) / effective_damage(defender)
    #defender_hp = project.get_hp(defender) / effective_damage(attacker, defender)
    #num_hits = math.ceil(defender_hp)
...
def battle(pkmn1, pkmn2):
    ...
    attacker = pkmn1
    defender = pkmn2
    if num_hits(attacker, defender) == "infinitely many":
        if num_hits(defender, attacker) == "infinitely many":
            return "Draw"
        else:
            return defender
    if num_hits(defender, attacker) == "infinitely many":
        ...
        return attacker 
    elif project.get_speed(defender) > project.get_speed(attacker):
        return defender
    elif project.get_speed(defender) < project.get_speed(attacker):
        return attacker  
    else:
...
def get_stat_total(pkmn):
    pk_hp = project.get_hp(pkmn)
    attack = project.get_attack(pkmn)
    defense = project.get_defense(pkmn)
    special_attack = project.get_sp_atk(pkmn)
    special_defense = project.get_sp_def(pkmn)
    speed = project.get_speed(pkmn)
    
    total_stat = (pk_hp + attack + defense + special_attack + special_defense + speed)
...
def friendship_score(pkmn1, pkmn2):
    ...
    if project.get_type1(pkmn1) == project.get_type1(pkmn2):
        if project.get_type2(pkmn1) != "DNE" and project.get_type2(pkmn1) == project.get_type2(pkmn2):
            score += 3
        else:
            score += 1
    elif project.get_type2(pkmn1) != "DNE" and project.get_type2(pkmn1) == project.get_type2(pkmn2):
        score += 1
