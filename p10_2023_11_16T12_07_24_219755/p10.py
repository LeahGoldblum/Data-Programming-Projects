...
lum_total = 0
lum_count = 0
for stars in stars_1_dict:
    star_lum = stars_1_dict[stars].stellar_luminosity
    if star_lum == None:
        continue 
    lum_total += star_lum
    lum_count += 1
    
avg_lum_stars_1 = lum_total/lum_count
avg_lum_stars_1
# -
grader.check("q7")
...
age_total = 0
age_count = 0
for stars in stars_2_dict:
    star_age = stars_2_dict[stars].stellar_age
    if star_age == None:
        continue 
    age_total += star_age
    age_count += 1
...
avg_age_stars_2 = age_total/age_count
avg_age_stars_2
# -
grader.check("q8")
...
for starname in stars_dict: 
    star_radius = stars_dict[starname].stellar_radius
    if star_radius == None:
        continue
    if biggest_star == None:
        biggest_star = starname
    if stars_dict[biggest_star].stellar_radius<star_radius:
        biggest_star = starname
biggest_star
...
for starname in stars_dict:
    ...
    if star.stellar_age == None:
        continue 
    if starname.startswith("Kepler"):
        star_age += star.stellar_age
        star_count += 1
avg_age_kepler = star_age/star_count
avg_age_kepler
...
last_five_planets_1 = allplanets_1[-5:]
last_five_planets_1                         
...
for planets in allplanets_2:
    if planets.controversial_flag is True:
        controversial_planets.append(planets)
controversial_flag
grader.check("q14")
...
last_five_planets_4 = allplanets_4[-5:]
last_five_planets_4
# -
grader.check("q15")
...
for planets in planets_list:
    if planets.discovery_year == 2023:
        planets_disc_2023.append(planets)
planets_disc_2023 = len(planets_disc_2023)
planets_disc_2023
grader.check("q17")
...
for planets in planets_list:
    if planets.planet_name == "TOI-2202 c":
        star_name = planets.host_name
        break
toi_2022_c_star = stars_dict[star_name]
...
for planet in planets_list: 
    ...
    if planet_radius == None:
        continue
    star_name = planet.host_name
    stellar_radius = stars_dict[star_name].stellar_radius
    if stellar_radius == None or stellar_radius <= 10:
        continue
    star_count += 1
    avg_planet_radius_big_stars += planet_radius
    
avg_planet_radius_big_stars /= star_count
...
#two for loops, find youngest star, second is put the star in to  a list of the host name is equal to the youngest star
youngest_star = None
youngest_star_name = None

for star_name in stars_dict:
    star = stars_dict[star_name]
    if star.stellar_age == None:
        continue
    if youngest_star == None:       
        youngest_star = star.stellar_age
        youngest_star_name = star_name
    if youngest_star >= star.stellar_age:
        youngest_star = star.stellar_age
        youngest_star_name = star_name
...
for planet in planets_list:
    if planet.host_name == youngest_star_name:
