...
file_paths = [os.path.join("data", filename) for filename in files_in_data]

csv_file_paths = [os.path.join("data", files) for files in files_in_data if files.endswith(".csv")]

stars_paths = [os.path.join("data", starfiles) for starfiles in files_in_data if starfiles.startswith("stars")]
...
def star_cell(row_idx, col_name, stars_rows, header=stars_header):
    ...
    if val == "":
        return None
    elif col_name in ["Star Name", "Spectral Type"]:
        return val
    else:
        val = float(val)
    return val
...
def get_stars(stars_file):
    stars_data = process_csv(stars_file)
    stars_header = stars_data[0]
    stars_1_rows = stars_data[1:]
    stars = {} # initialize empty dictionary to store all stars
...
stars_dict = {}
for star_file in stars_paths:
    new_star_files = get_stars(star_file)
    stars_dict.update(new_star_files) 
...
planets_list = []
allplanets_1 = get_planets(os.path.join("data", "planets_1.csv"), os.path.join("data", "mapping_1.json"))
allplanets_2 = get_planets(os.path.join("data", "planets_2.csv"), os.path.join("data", "mapping_2.json"))
allplanets_3 = get_planets(os.path.join("data", "planets_3.csv"), os.path.join("data", "mapping_3.json"))
allplanets_4 = get_planets(os.path.join("data", "planets_4.csv"), os.path.join("data", "mapping_4.json"))
allplanets_5 = get_planets(os.path.join("data", "planets_5.csv"), os.path.join("data", "mapping_5.json"))

planets_list.extend(allplanets_1)
planets_list.extend(allplanets_2)
planets_list.extend(allplanets_3)
planets_list.extend(allplanets_4)
planets_list.extend(allplanets_5)
...
for planet in planets_list: 
    if planet.host_name == "GJ 9827":
        orbit_sqr = planet.orbital_period ** 2
        radius_cube = planet.semi_major_radius **3
        k_ratio = orbit_sqr/radius_cube
        ratios_gj9827.append(k_ratio)
ratios_gj9827
...
gj9827_stdev = statistics.stdev(ratios_gj9827)
gj9827_mean = statistics.mean(ratios_gj9827)

coeff_gj9827 = gj9827_stdev/gj9827_mean
coeff_gj9827
# -
...
predict_sm = 133408/gj9827_mean
for planet in planets_list: 
    if planet.host_name == "GJ 9827":
        star_name = planet.host_name
        gj_9827_sm = stars_dict[star_name].stellar_mass
        
percentage_change = ((predict_sm - gj_9827_sm)/(gj_9827_sm))*100
percentage_change
# -
...
for planet in planets_list:
    ...
    temp_4th_power = equilibrium_temperature ** 4
...
for key, val in stars_dict.items():
    if val.stellar_mass==None or val.stellar_mass < 0.3:
        continue 
    if 0.3 <= val.stellar_mass < 8:
        star_classes['Red Giant'].append(key)
    elif 8 <= val.stellar_mass < 10.5:
        star_classes['White Dwarf'].append(key)
    else:
        star_classes['Neutron Star'].append(key)
...
for star_class in star_classes:
    star_lum = []
    for starname in star_classes[star_class]:
        
        star = stars_dict[starname]
#     for starname in stars_dict:
#         star = stars_dict[starname]
        if star.stellar_luminosity == None: 
            continue

        luminosity = star.stellar_luminosity
        star_lum.append(luminosity)

    avg_star_lum = sum(star_lum)/len(star_lum)
            
    star_classes_avg_lum[star_class] = avg_star_lum
...
# for stars in star_classes[star_class]:
#     stars = stars_dict[star_class]
#     if stars.stellar_mass == None or stars.stellar_radius == None or stars.stellar_luminosity == None:
#         continue
#         star_mass = stars.stellar_mass
#         star_radius = stars.stellar_radius
#         lum_dict[stars].append(stars.stellar_luminosity)
#         star_density = star_mass/(star_radius **3)
#         density_dict[stars].append(star_density)
...
#         stars = stars_dict.star_class
    for starname in star_classes[star_class]:
#         print(starname)
        stars = stars_dict[starname]
#         print(stars)
#         print(lum_dict.keys())
        stellar_mass = stars.stellar_mass
        stellar_radius = stars.stellar_radius
        stellar_luminosity = stars.stellar_luminosity
#         print(stellar_mass, stellar_radius, stellar_luminosity)
        
        if stellar_mass == None or stellar_radius == None or stellar_luminosity == None:
            continue
        lum_dict[star_class].append(stellar_luminosity)
            
        stellar_density = stellar_mass/(stellar_radius **3)
        density_dict[star_class].append(stellar_density)
...
#         print(stellar_mass, stellar_radius, stellar_luminosity)
        ...
        stellar_density = stellar_mass/(stellar_radius **3)
        if stellar_density <= 25:
            lum_dict_no_outliers[star_class].append(stellar_luminosity)
            density_dict_no_outliers[star_class].append(stellar_density)
...
for star in stars_dict:
    stars = stars_dict[star]
    effective_temp = stars.stellar_effective_temperature
    luminosity = stars.stellar_luminosity
    star_age = stars.stellar_age
    star_mass = stars.stellar_mass
    if effective_temp is None or luminosity is None or star_age is None or star_mass is None:
        continue


    temp_list.append(effective_temp)
    lum_list.append(luminosity)
...
def get_all_paths_in(directory):
    ...
    paths = []
    all_files_in_dir = [files for files in os.listdir(directory) if not files.startswith(".")]
    for file in all_files_in_dir:
        all_paths = os.path.join(directory, file)
        if os.path.isdir(all_paths):
            paths.extend(get_all_paths_in(all_paths))
        else:
...
for paths in broken_data:
    new_planets = get_planets(os.path.join("data", "planets_5.csv"), paths)
...
def get_surface_gravity(planet):
    if planet.planet_mass is None or planet.planet_radius is None:
        return None
    g_force_ratio = planet.planet_mass / (planet.planet_radius ** 2)
    return g_force_ratio
...
def get_distances_to_star(planet):
    planet_dist = []
    if planet.eccentricity is None or planet.semi_major_radius is None:
        return None
    eccentricity = planet.eccentricity
    semi_major_radius = planet.semi_major_radius
    
    𝚜𝚑𝚘𝚛𝚝𝚎𝚜𝚝_𝚍𝚒𝚜𝚝 = semi_major_radius * (1 - 𝚊𝚋𝚜(eccentricity))
    𝚕𝚘𝚗𝚐𝚎𝚜𝚝_𝚍𝚒𝚜𝚝 = semi_major_radius * (1 + 𝚊𝚋𝚜(eccentricity))
    planet_dist.append(shortest_dist)
    planet_dist.append(longest_dist)
    return planet_dist
...
def get_liquid_water_distances(planet):
        ...
        if star.stellar_luminosity is None:
            return None 
        liquid_water_short_dist = statistics.sqrt((10 ** star.stellar_luminosity) / 1.15)
        liquid_water_long_dist = statistics.sqrt((10 ** star.stellar_luminosity) / 0.53)
        liquid_water_list.append(liquid_water_short_dist)
        liquid_water_list.append(liquid_water_long_dist)
        return liquid_water_list
...
for planet in all_planets_list:
    actual_short_long_dist = get_distances_to_star(planet)
    short_long_liquid_dist =  get_liquid_water_distances(planet)

    if actual_short_long_dist and short_long_liquid_dist:
#         if actual_short_long_dist > short_long_liquid_dist:
#             if actual_short_long_dist < short_long_liquid_dist:

        if (actual_short_long_dist[0] >= short_long_liquid_dist[0]): 
            if (actual_short_long_dist[1] <= short_long_liquid_dist[1]):
...
def get_surface_temperatures(planet):
    surface_temp = []
    equil_temp = planet.equilibrium_temperature
    if equil_temp == None:
        return None
    max_surface_temp = ((1-0.0) ** (1/4)) * equil_temp
    min_surface_temp = ((1-0.5) ** (1/4)) * equil_temp
    surface_temp.append(min_surface_temp)
    surface_temp.append(max_surface_temp)
...
for planet in all_planets_list:
    ...
    if surface_temp[0] > 263:
        if surface_temp[1] < 323:
...
for planet in all_planets_list:
    ...
    if gravity is None:
        continue
    if not (0.75 < gravity < 1.25):
        continue 
    
    actual_dist = get_distances_to_star(planet)
    liquid_water_dist = get_liquid_water_distances(planet)
    if actual_dist is None or liquid_water_dist is None:
        continue
        
    if not(actual_dist[0] >= liquid_water_dist[0]): 
        continue
    if not(actual_dist[1] <= liquid_water_dist[1]):
        continue
        
    surface_temp = get_surface_temperatures(planet)
    if surface_temp is None:
        continue
    if not (200 < surface_temp[0] < 350):
        continue
