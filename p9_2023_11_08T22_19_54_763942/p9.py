...
def get_mapping(path):    
    ...
    for column in csv_data:
        map_value.update({column[0] : column [1]})
    return map_value
...
def get_raw_movies(path):
    ...
    for row in csv_rows:
        raw_movies_dict = {}
        for idx in range(len(csv_header)):
            if csv_header[idx] in ['year', 'duration']:
                raw_movies_dict[csv_header[idx]] = int(row[idx])
            elif csv_header[idx] in ['rating']:
                raw_movies_dict[csv_header[idx]] = float(row[idx])
            elif csv_header[idx] in ['title']:
                raw_movies_dict[csv_header[idx]] = (row[idx])
            else:
                raw_movies_dict[csv_header[idx]] = (row[idx]).split(', ')
        movie_list.append(raw_movies_dict)
    return movie_list
...
def get_movies(movies_path, mapping_path):
    ...
    for movie in movies:
        ...
        for director_id in movie['directors']:
            director_name = mapping_dict[director_id]
            directors.append(director_name)
        movie['directors'] = directors
        
        cast_members = []
        
        for cast_member_id in movie['cast']:
            cast_name = mapping_dict[cast_member_id]
            cast_members.append(cast_name)
        movie['cast'] = cast_members

        title_id = movie['title']
        title_name = mapping_dict[title_id]
        movie['title'] = title_name
    return movies
...
clint_e = find_specific_movies(movies, "Clint Eastwood")
sergio_l = find_specific_movies(movies, "Sergio Leone")
ce_sl_movie_ratings = []
for movie in clint_e:
    for movie_sl in sergio_l:
        if movie["title"] == movie_sl["title"]:
            ce_sl_movie_ratings.append(movie['rating'])
median_eastwood_leone_rating = median(ce_sl_movie_ratings)
...
rated_toy_story_movies = []
highest_rated_toy_story_movies = []
for movie in toy_story_movies:
    toy_rating = movie["rating"]
    rated_toy_story_movies.append(toy_rating)
highest_toy_rating = max(rated_toy_story_movies)
for movie in toy_story_movies:
    if movie['rating'] == highest_toy_rating:
...
def bucketize(movie_list, category):
    ...
    for movie in movie_list:
        ...
        if type(category_value) != list:  
            cat_val = []
            cat_val.append(category_value)
            category_value = cat_val
        for categories in category_value:
            if categories not in buckets:
                buckets[categories] = []
            buckets[categories].append(movie)
    return buckets
...
#all_robbie_movies = find_specific_movies(movies, "Margot Robbie")
robbie_movies = cast_buckets["Margot Robbie"]
# robbie_movies= all_cast_movies["Margot Robbie"]
robbie_movies 
...
movie_genres = bucketize(movies, 'genres')
genre_num = {moviegen:len(movie_genres[moviegen]) for moviegen in movie_genres}
...
# kubrick_genres = {movie: len(movie["genres"]) for movie in sk_dir_movies}

sk_dir_movies = director_buckets['Stanley Kubrick']
sk_genres = bucketize(sk_dir_movies, 'genres')
kubrick_genres = {sk_moviegen:len(sk_genres[sk_moviegen]) for sk_moviegen in sk_genres}
kubrick_genres
...
# mov_year = sci_fi_movies['year']
# decades = year_to_decade(mov_year)
# sci_fi_decade_mapping = {decades:len(sci_fi_movies[decades]) for sfi_movie in sci_fi_movies}
...
for sfi_movie in sci_fi_movies:
    mov_year = sfi_movie['year']
    decades = year_to_decade(mov_year)
    if decades not in sci_fi_decade_mapping:
        sci_fi_decade_mapping[decades] = 1
    else:
        sci_fi_decade_mapping[decades] += 1
sci_fi_decade_mapping
...
# do NOT plot just yet
jc_dir_movies = director_buckets["James Cameron"]
jc_genres = bucketize(jc_dir_movies, 'genres')
ratings_by_genre = {}
for genre in jc_genres:    
    ratings_by_genre[genre] = [movie['rating'] for movie in jc_genres[genre]]
    
cameron_median_genres = {}
cameron_median_genres = {genre: median(ratings_by_genre[genre]) for genre in ratings_by_genre}
cameron_median_genres
...
cameron_genres_desc = sorted(cameron_median_genres, key = lambda c:cameron_median_genres[c], reverse = True)
...
# ms_dir_mov = director_buckets['Martin Scorsese']
# scorsese_de_niro_movies = []
# scorsese_de_niro = {}
# for ms_mov in ms_dir_mov:
#     scorsese_de_niro[ms_mov] = [ms_cast['Robert De Niro'] for ms_cast in ms_mov['cast']]
#     scorsese_de_niro_movies.append(scorsese_de_niro[ms_mov])
# scorsese_de_niro_movies = sorted(scorsese_de_niro_movies, key=lambda m:m['year'])
# scorsese_de_niro_movies
# -
...
scorsese_de_niro_movies = []
for ms_mov in ms_dir_mov:
    for ms_cast in ms_mov['cast']:
        if "Robert De Niro" in ms_cast:
            scorsese_de_niro_movies.append(ms_mov)
scorsese_de_niro_movies = sorted(scorsese_de_niro_movies, key=lambda m:m['year'])
scorsese_de_niro_movies
...
for year in year_buckets:
    if year < 2020:
        pre_twt_movie.extend(year_buckets[year])
pre_twt_directs = bucketize(pre_twt_movie, 'directors')        
best_directors = []
for directors in pre_twt_directs:
    if len(pre_twt_directs[directors]) >= 10:
        dir_ratings = [movie['rating'] for movie in pre_twt_directs[directors]]
        if median(dir_ratings) >= 7.5 and min(dir_ratings)>=5.0:
            best_directors.append(directors)
best_directors
